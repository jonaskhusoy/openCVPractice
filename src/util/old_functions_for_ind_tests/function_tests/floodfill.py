...
# img = cv2.imread(path) #cv2 img object
# img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) #convert color scheme to HSV for easier color separation

# ret, mask = cv2.threshold(img_gray, 250, 255, cv2.THRESH_BINARY)

# result = img.copy() #new image to create contours on
...
# cv2.floodFill(result, mask2, (0,0), 255)
